...
class HrOnboardingService:
    ...
    def _validate_and_onboard_one(
        ...
        # 6. Update users table. HR is the authority on teams and CAN set them here (Gaurav
        # 2026-08-07) — the form preselects the user's existing teams so they aren't lost.
        # users.teams is a Postgres ARRAY (text[]): pass a Python list so psycopg2 adapts it
        # (a comma-joined string → "malformed array literal"). If teams is omitted from the
        # payload (None), DON'T touch the column — preserve whatever is already defined.
        set_teams = teams is not None
