...
| 0e | A-14 | ✅ **LIVE ENGINE RUN (Gaurav's go, 2026-07-25):** full ladder on all 761 staged txns (2 passes; AI on). **721/761 = 95% auto-handled**: 285 party-default · 51 transfer-awaiting-counterpart · 26 rule · 35 AI≥0.80 (DRAFT JE) · **324 AI low-conf → NEEDS_REVIEW (suggested code + reasoning stored)** · 40 Pending (no route). 466 enriched. Route recorded per txn (JE `source` + ai fields). Report: `wip/reconciliation/engine_run_20260725_report.csv`. **Pass-1 bugs found+fixed:** AI phase died on one bad chunk (now chunk-independent) · `_entity_short` returned 'Ltd' for all live legal names → IC pairs NEVER matched in production (fixed + test). **Next: review walk of the 324 + the 40** | done | Follow-up (Gaurav, 2026-07-25): 136 system-written counterparty_names restored to raw import values from `original_csv_row` (0 diffs remain; 427/755 txns carry a source-provided name). Found: old OCBC **CSV** import mapped a DATE column into counterparty_name (adapter bug → A-12 scope) | done |
