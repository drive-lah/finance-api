...
| S-3a | **Rules engine change (queued)** | Enforce POL-17 in code: `_apply_rule` ignores `counterparty_name/type` actions; rule create/update API rejects them; later migration drops the columns. Plus fix the pipe-pattern bug (53 live rules with `'x | y'` never matched — engine needs split-on-`|` or rules rewritten as singles). |
| S-3 | **Rules** | ✅ **REVIEW COMPLETE (2026-07-24)** — all 58 walked one-by-one. **Final rule book = 67 rules**: 21 transfer (incl. 1 new mined Stripe-settlement) · 39 pure-text · 4 kept exceptions (apple.com→6701, GSuite/Play/Storage→6701) · 2 party-no-default · vs **178 live rules → INACTIVE** (171 redundant-to-defaults, 6 superseded by arbitrated defaults, 1 identity-only). Many walk answers flipped PARTY defaults instead (POL-13 both ways). Manifest: `wip/reconciliation/live_rules_manifest.csv` (per-rule fates). At apply: deactivations + strip counterparty actions (POL-12/17) + pipe-split rewrites (DQ-9) + entity-scoping pass (DQ-7). |
