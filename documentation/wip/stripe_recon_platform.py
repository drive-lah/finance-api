...
s=get_session_factory()(); P=sys.argv[1] if len(sys.argv)>1 else '2026-01-01'; N='2026-02-01'
...
for eid,MK,acc,bt,tf,ca,TZ in [(2,'SG','1017','sg_stripe_balance_transactions','sg_stripe_transfers','sg_stripe_connected_accounts','UTC'),
    ...
    imp['host_transfers_cash']=v(f"SELECT amount FROM v_{MK}_c_host_transfers WHERE month='{P}'")
    imp['connect_internal_transfer']=-v(f"SELECT amount FROM v_{MK}_c_host_rms_internal_clearing WHERE month='{P}'")
    imp['disputes']=-v(f"SELECT amount FROM v_{MK}_c_disputes WHERE month='{P}'")  # flip=True -> impact=-view
    imp['stripe_fees']=-v(f"SELECT amount FROM v_{MK}_c_stripe_fees_paid WHERE month='{P}'")  # Cr platform = -fee
    imp['stripe_platform_adjustments']=-v(f"SELECT amount FROM v_{MK}_c_stripe_platform_adjustments WHERE month='{P}'")  # flip=True
